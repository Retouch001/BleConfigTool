...
static const NSInteger RT_CENTRAL_MANAGER_INIT_WAIT_SECOND = 2.0;//CBcentralManager等待设备打开间隔时间
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral{
    ...
    if ([self.delegate respondsToSelector:@selector(deviceServiceUUID)]) {
        serviceUUID = [self.delegate deviceServiceUUID];
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    BLELog(@">>>>>>>>>>扫描到设备: %@ 的特征：%@\n",peripheral.name,service.characteristics);
    if (error) {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(nonnull CBCharacteristic *)characteristic error:(nullable NSError *)error{
    BLELog(@">>>>>>>>>>设备: %@ 特征值发生改变：%@",peripheral.name,characteristic.value);
    if (error) {
